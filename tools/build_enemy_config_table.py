...
ENEMY_ROWS = [
    ["bug", "BUG", "第一层", "普通/教学", 28, "攻击[6] / 防御[5]", "行动循环: 攻击 -> 防御 -> 攻击+防御", "第一层基础攻防教学，验证敌方代码、多行意图和护盾窗口", "img/enemies/bug.png", "已接入"],
    ["error", "Error", "第一层", "普通", 30, "攻击[7-10]", "每回合只攻击；每次受到攻击有 10% 概率下回合眩晕", "高一点的随机攻击压力，鼓励玩家主动攻击并理解打断机会", "img/enemies/error.png", "已接入"],
    ["loop", "回环", "第一层", "普通", 32, "①循环(3){攻击[3]} / ②循环(2){攻击[3] + 防御[3]}", "通过固定多段行动引入循环机制；第二段循环同时攻击与防御", "让玩家第一次理解循环不是抽象语法，而是重复执行的敌方行动", "img/enemies/loop.png", "已接入"],
    ["branch", "岔路", "第一层", "普通", 34, "①攻击[3]；如果(你没有护盾) 攻击[5]；如果(自身HP<30%) 攻击[3] / ②如果(你有护盾) 防御[5]；否则 下次攻击参数+1", "根据玩家护盾与自身血量走不同分支；无护盾时追击，低血量时补刀，有护盾时转防御，否则蓄积参数", "教玩家用防御影响敌方分支，建立 if/else 的战斗直觉", "img/enemies/branch.png", "已接入"],
    ["ddl", "DDL", "第一层", "精英", 42, "攻击[6] / 易伤[1] / 攻击[12]", "倒计时式精英；先施加 1 层易伤，再用大攻击制造截止压力", "教玩家处理易伤与预告爆发，提前防御或爆发击杀", "img/enemies/ddl.svg", "已接入"],
    ...
    ["syntaxerr", "红字", "第一层", "Boss", 125, "防御[10] / 攻击[10-15] / 语法错误!", "周期性护盾、攻击、随机弃 1 张手牌", "第一层综合检验 Boss", "img/enemies/syntaxerr.svg", "已实装"],
