...
ENEMY_ROWS = [
    ...
    ["loop", "回环", "第一层", "普通", 32, "循环(2){攻击[3]} / 防御[5] / 循环(3){攻击[2]}", "通过低伤害多段行动引入循环机制", "让玩家第一次理解循环不是抽象语法，而是重复执行的敌方行动", "img/enemies/loop.png", "素材已生成；机制待接入"],
    ["branch", "岔路", "第一层", "普通", 34, "如果(你无护盾) 攻击[9] / 否则 攻击[5]", "根据玩家状态改变行动，引入判断机制", "教玩家用防御影响敌方分支，建立 if/else 的战斗直觉", "img/enemies/branch.png", "素材已生成；机制待接入"],
...
FIRST_LAYER_ROWS = [
    ...
    ["回环", "第一层第 3-4 列后加入", "中低", "HP 32；低伤害多段行动，引入循环机制"],
    ["岔路", "第一层第 3-4 列后加入", "中", "HP 34；根据玩家是否有护盾选择攻击强度，引入判断机制"],
