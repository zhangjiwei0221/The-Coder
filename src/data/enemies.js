...
export const ENEMIES = {
  bug:{name:'Bug',icon:'🐛',hp:28,tier:1,getIntent(t){if(t%3===2)return{type:'buff',val:5,desc:'获得5护盾',code:'防御[5]'};if(t%3===0)return{type:'atk',val:6,code:'攻击[6]\n防御[5]',totalDmg:6,totalShield:5};return{type:'atk',val:6,code:'攻击[6]'};}},
  typo:{name:'错字虫',icon:'📝',hp:26,tier:1,getIntent(t){const v=rand(5,7);return{type:'atk',val:v,code:`攻击[${v}]`};}},
  redpoint:{name:'红点提醒',icon:'🔴',hp:30,tier:1,getIntent(t){if(t%3===0)return{type:'debuff',val:0,desc:'下回合少抽1张牌',code:'提醒弹出()',drawPenalty:{any:1}};return{type:'atk',val:5,code:'攻击[5]'};}},
  nullptr:{name:'空值幽影',icon:'⚠️',hp:38,tier:1,getIntent(t){if(t%3===2)return{type:'buff',val:0,desc:'潜伏一回合',code:'// 空值潜伏'};if(t%3===0)return{type:'atk',val:11,code:'攻击[11]'};return{type:'atk',val:7,code:'攻击[7]'};}},
  ...
  syntaxerr:{name:'红字审判',icon:'🚫',hp:125,tier:3,getIntent(t){if(t%4===0)return{type:'buff',val:10,desc:'获得10护盾',code:'防御[10]'};if(t%4===2)return{type:'debuff',val:1,desc:'随机弃掉1张手牌',code:'语法错误!'};const v=rand(10,15);return{type:'atk',val:v,code:`攻击[${v}]`};}},
...
export const ENEMY_UI_META = {
  ...
  错字虫: { codeLib:['攻击[5-7]'], ability:'每回合造成小幅随机伤害。' },
  红点提醒: { codeLib:['攻击[5]','提醒弹出()'], ability:'每3回合干扰一次，下回合少抽1张牌。' },
  空值幽影: { codeLib:['攻击[7]','// 空值潜伏','攻击[11]'], ability:'会潜伏一回合，随后打出更高伤害。' },
