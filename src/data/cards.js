...
export const CARD_DEFS = {
  ...
  heal:{type:'instruction',name:'治疗',icon:'💚',desc:'回复X点HP。本战移出',needsParam:true,consume:true,rarity:2,exec(ctx,v){ctx.healHP(v);}},
  draw:{type:'instruction',name:'抽牌',icon:'🃏',desc:'抽取X张牌',needsParam:true,rarity:2,exec(ctx,v){ctx.drawCards(v);}},
  charge:{type:'instruction',name:'蓄力',icon:'⚡',desc:'下回合所有攻击伤害+50%(不叠加)',needsParam:false,rarity:1,exec(ctx){ctx.bstate.chargeDamageNextTurn=true;}},
  poison:{type:'instruction',name:'毒素',icon:'🧪',desc:'敌方回合结束受到X伤害,持续X回合',needsParam:true,rarity:2,exec(ctx,v){ctx.bstate.enemy.status.poison+=v;}},
  burn:{type:'instruction',name:'燃烧',icon:'🔥',desc:'敌方行动前受到X伤害,持续X回合',needsParam:true,rarity:2,exec(ctx,v){ctx.bstate.enemy.status.burn+=v;}},
  shield:{type:'instruction',name:'壁垒',icon:'🧱',desc:'获得X+3护盾。本战移出',needsParam:true,consume:true,rarity:2,exec(ctx,v){ctx.addShield(v+3);}},
  reflect:{type:'instruction',name:'反弹',icon:'🪞',desc:'反弹接下来X点伤害。本战移出',needsParam:true,consume:true,rarity:3,exec(ctx,v){ctx.bstate.player.status.reflect+=v;}},
  weakenCard:{type:'instruction',name:'干扰',icon:'📉',desc:'敌人下回合每段攻击-X',needsParam:true,rarity:1,exec(ctx,v){ctx.bstate.enemy.status.weaken+=v;}},
  weakAll:{type:'instruction',name:'压制',icon:'🧯',desc:'敌人本回合攻击-30%。本战移出',needsParam:false,consume:true,rarity:2,exec(ctx){ctx.bstate.enemyWeakThisTurn=true;}},
  vuln:{type:'instruction',name:'易伤',icon:'🎯',desc:'敌人接下来X次受攻击伤害+50%',needsParam:true,rarity:2,exec(ctx,v){ctx.bstate.enemy.status.vulnerable+=v;}},
  multiHit:{type:'instruction',name:'连击',icon:'👊',desc:'造成X次1点伤害,适合触发易伤',needsParam:true,rarity:3,exec(ctx,v){for(let i=0;i<v;i++)ctx.dealDamage(1);}},
  doubleStrike:{type:'instruction',name:'双击',icon:'⚡',desc:'造成X点伤害两次。本战移出',needsParam:true,consume:true,rarity:1,exec(ctx,v){ctx.dealDamage(v);ctx.dealDamage(v);}},
  lifesteal:{type:'instruction',name:'吸血',icon:'🧛',desc:'造成X伤害,回复一半HP。本战移出',needsParam:true,consume:true,rarity:3,exec(ctx,v){ctx.dealDamage(v);ctx.healHP(Math.floor(v/2));}},
  // SYNTAX CARDS
  ...
  if_bal2:{type:'instruction',name:'如果{+5}...则',icon:'➕',desc:'效果+5',subtype:'if',bonus:{type:'add',val:5},capacity:1,needsParam:false,rarity:2,draftable:false},
  if_hunter3:{type:'instruction',name:'如果{×2}...则',icon:'✖️',desc:'效果×2',subtype:'if',bonus:{type:'mul',val:2},capacity:1,needsParam:false,rarity:3,draftable:false},
